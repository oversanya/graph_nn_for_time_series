...
class GNNLayer(nn.Module):
    def __init__(self, input_dim, output_dim, use_skip=True):
        """
        Args:
            input_dim: размерность входных признаков
            output_dim: размерность выходных признаков
            use_skip: использовать ли skip connection
        """
    ...
    def forward(self, A, X_prev):
        # Основное преобразование: AXW
        AX = torch.matmul(A, X_prev)
        X_k = torch.matmul(AX, self.weight) + self.bias
        
        # Skip connection
        if self.use_skip:
            if self.skip_linear is not None:
                X_prev = self.skip_linear(X_prev)
            X_k = X_k + X_prev
